...
func play_sanctuary_ambient(biome: String) -> void:
        ...
        if _ambient_drone_player.stream is AudioStreamGenerator:
                ...
                var buf: AudioStreamGeneratorPlayback = _ambient_drone_player.get_stream_playback() as AudioStreamGeneratorPlayback
                if buf:
                        var frames_to_push := mini(samples.size(), buf.get_frames_available())
                        if frames_to_push > 0:
                                buf.push_buffer(samples.slice(0, frames_to_push))
